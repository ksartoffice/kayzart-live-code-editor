...
export async function openKayzartEditor(page: Page) {
  await login(page);
  await page.goto(convertUrl(), { waitUntil: 'domcontentloaded' });
  ...
  if (!(await page.evaluate(editorReady).catch(() => false))) {
    ...
    if ((await submit.count()) > 0) {
      ...
      throw new Error(
        `Could not open post ${postId} in the Kayzart editor. ` +
